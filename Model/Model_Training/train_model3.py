...
BASE_DATA_DIR = "D:\\Downloads\\beamng_pics\\beamng_pics2" 
DATA_CSV_FILE_PATH = os.path.join(BASE_DATA_DIR, 'steering_labels.csv')

SAVE_DIR = './output3/'
MODEL_SAVE_PATH = os.path.join(SAVE_DIR, 'ai_driver_cnn_deep_3.pth') 

MAX_STEERING_ANGLE_DEGREES = 540.0
...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, csv_file_path, base_image_dir, transform = None):
        ...
        for index, row in data_df.iterrows():
            ...
            self.examples.append((center_img_file, steering_angle))


    def __getitem__(self, index):
        """Retrieves one data point and applies dynamic augmentation ONLY."""
        center_file, steering_angle = self.examples[index]

        # Apply augmentation with 60% probability
        if np.random.rand() < 0.6:
            image, steering_angle = augment(self.base_image_dir, center_file, steering_angle)
        else:
